...
const Tours = (props) => {
	...
	let [tourState, setTour] = useState(toursState)
	let tourElement;
	if (props){
		tourElement = tourState.map(item=>{
			return <TourComponent img={item.img} title={item.title} price={item.price} text={item.text} id={item.id} tour={tourState} setTours={setTour}/>
		})
	}
	return (
		<div className='tours'>
			{tourState.length>0 ? tourElement : <button onClick={()=>setTour(toursState)}>Refresh</button>}
		</div>
	);
};
